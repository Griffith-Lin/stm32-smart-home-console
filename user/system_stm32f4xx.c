...
{
    // ==== µÚÒ»¼þÊÂ£ºÀ­µÍ PB15£¬¸´Î» WS2812E Á´ ====
    RCC->AHB1ENR |= RCC_AHB1ENR_GPIOBEN;   // ¿ª GPIOB Ê±ÖÓ
    __NOP(); __NOP();                         // µÈÊ±ÖÓÎÈ¶¨
    GPIOB->MODER &= ~(3 << 30);              // PB15 Ä£Ê½ÇåÁã
    GPIOB->MODER |=  (1 << 30);              // PB15 = Êä³öÄ£Ê½
    GPIOB->ODR   &= ~(1 << 15);              // PB15 Êä³öµÍµçÆ½
    // ´Ó´Ë¿ÌÆð PB15 ¾Í±»Çý¶¯Îª LOW£¬WS2812E ½øÈë¸´Î»×´Ì¬
