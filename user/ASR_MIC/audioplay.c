...
//	printf("%s\r\n",buff);
//	LCD_Font_Dis_Str(100,60,24,RED,WHITE,buff);
}
/*********************************************************************************************************
* 函 数 名 : Audio_MsgShow
* 功能说明 : 显示播放时间
...
static DIR            wavdir;
static FILINFO        wavfileinfo;
static unsigned char *pname;
static unsigned short *wavindextbl;
static unsigned short totwavnum;
static unsigned short curindex;
...
void Audio_MusicInit(void)
{
    unsigned char res;
    unsigned short temp;
    unsigned char *fn;

    while (f_opendir(&wavdir, "0:/MUSIC")) { printf("MUSIC文件夹错误!\r\n"); Delay_Ms(200); }
    totwavnum = Audio_GetTnum("0:/MUSIC");
    while (totwavnum == NULL) { printf("没有音乐文件!\r\n"); Delay_Ms(200); }

    wavfileinfo.fsize = FF_MAX_LFN * 2 + 1;
    pname = mymalloc(SRAMIN, wavfileinfo.fsize);
    wavindextbl = mymalloc(SRAMIN, 2 * totwavnum);
    while (pname == NULL || wavindextbl == NULL) { printf("内存分配失败!\r\n"); Delay_Ms(200); }
    ...
    if (res == FR_OK)
    {
        ...
        while (1)
        {
            ...
            fn = (unsigned char *)(*wavfileinfo.fname ? wavfileinfo.fname : wavfileinfo.fname);
            res = f_typetell(fn);
            if ((res & 0XF0) == 0X40) { wavindextbl[curindex] = temp; curindex++; }
        }
    }
    curindex = 0;
...
u8 Audio_MusicStep(void)
{
    ...
    if (wavindextbl == NULL) return 0;   /* 未 init：不自动开歌 */

    if (wav_phase == WAV_STOPPED)                /* 停止态：等播放请求才重开 */
    {
        if (status_dev.PlayState != PLAY_PLAY) return 0;
        status_dev.PlayState = PLAY_CLEAR;       /* 直接开新曲，无需 Wav_PlayStep 消费 */
    }

    if (wav_phase == WAV_IDLE || wav_phase == WAV_STOPPED)      /* 无曲在播：打开 curindex 指向的曲 */
    {
        dir_sdi(&wavdir, wavindextbl[curindex]);
        res = f_readdir(&wavdir, &wavfileinfo);
        if (res != FR_OK || wavfileinfo.fname[0] == 0) return 1;
        fn = (unsigned char *)(*wavfileinfo.fname ? wavfileinfo.fname : wavfileinfo.fname);
        ...
        sprintf(music_name, "%s", fn);
        LCD_Font_Dis(30, 0, 16, RED, WHITE, (uint8_t*)music_name);
        Audio_IndexShow(fn, curindex + 1, totwavnum);

        key = Wav_PlaySong(pname);
        if (key != 0) return 1;
        return 0;
    }

    key = Wav_PlayStep();
    if (key == PLAY_PREVIOUS)       /* 上一曲 */
    {
        if (curindex) curindex--;
        else curindex = totwavnum - 1;
    }
    else if (key == PLAY_NEXT)      /* 下一曲 */
    {
        curindex++;
        if (curindex >= totwavnum) curindex = 0;
    }
    else if (key == WAV_END)              /* 本曲自然播完 → 自动下一首 */
    {
        curindex++;
        if (curindex >= totwavnum) curindex = 0;
    }
...
void Audio_MusicStart(void)         /* 云端/按键/触摸统一走这个入口 */
{
    Audio_MusicInit();              /* 一次性：目录/索引/内存分配 */
