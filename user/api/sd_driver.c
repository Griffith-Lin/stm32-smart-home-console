...
{	
    ...
    GPIO_SetBits(GPIOA,GPIO_Pin_4);      //  片选输出必须初始化为高(未选中),set的反义词是reset
...
{
	return SPI_Echo(data);
}	  
/*********************************************************************************************************
* 函 数 名 : SD_SPI_SpeedLow
* 功能说明 : 设置SPI低速
* 形    参 : 无
* 返 回 值 : 无
* 备    注 : D卡初始化的时候,需要低速
...
void SD_SPI_SpeedLow(void)
{
	...
	SPI1->CR1 |= SPI_BaudRatePrescaler_256;	//设置SPI1速度 
	SPI_Cmd(SPI1, ENABLE); 					//使能SPI1	
}
/*********************************************************************************************************
* 函 数 名 : SD_SPI_SpeedHigh
* 功能说明 : 设置SPI高速
* 形    参 : 无
* 返 回 值 : 无
* 备    注 : 无
...
{
	SPI1->CR1 &= 0XFFC7;					//位3-5清零，用来设置波特率
	SPI1->CR1 |= SPI_BaudRatePrescaler_16;	//设置SPI1速度 
	SPI_Cmd(SPI1, ENABLE); 					//使能SPI1	
}
/***********************************************END******************************************************/
...
* 功能说明 : 等待SD卡回应
* 形    参 : Response：要得到的回应值
* 返 回 值 : 0：得到希望的应答；其他：错误代码
* 备    注 : 无
*********************************************************************************************************/ 
...
{
	unsigned char r1 = 0;
	//SD_TYPE_V2HC 表示SD卡版本 2.0 及以上的高容量卡
	if(SD_Type != SD_TYPE_V2HC)	block <<= 9;	//转换为字节地址  <<9相当于*512
	if(cnt == 1)
	{
...
* 备    注 : 无
//块写指令说明
		0xFE：单块写（CMD24）或单块读（CMD17）操作的数据起始标记。
		0xFC：多块写（CMD25）操作的数据起始标记。
		0xFD：停止传输标记（用于结束多块写操作）。
*********************************************************************************************************/ 
unsigned char SD_WriteDisk(unsigned char*buf, unsigned int block, unsigned short cnt)
{
	...
	if(cnt == 1)	//单块写
	{
		r1 = SD_SendCmd(CMD24,block, 0X01);		//写命令  r1=0表示无错误
		if(r1 == 0)//指令发送成功
		{
	...
	{
		//SD_TYPE_MMC 用于标识当前检测到的存储设备为MMC卡而非SD卡,MMC卡是一种与SD卡类似但标准不同的闪存卡
		if(SD_Type != SD_TYPE_MMC)
		{
			SD_SendCmd(CMD55, 0, 0X01);	
			SD_SendCmd(CMD23, cnt, 0X01);			//发送指令	
		}
 		r1 = SD_SendCmd(CMD25, block, 0X01);		//连续写命令
		if(r1 == 0)
		{
			do
			{
				r1 = SD_SendBlock(buf, 0xFC);		//写入512个字节 先发前cnt-1块 最后一块需要发0xFD结束指令	 
				buf += 512;  
			}while(--cnt && r1==0);
			r1 = SD_SendBlock(0, 0xFD);				//写入512个字节 0xFD为结束指令 最后一帧要发结束指令 
		}
	}   
	SD_DisSelect();									//取消片选
	return r1;
