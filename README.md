...
| 2 | 按键长按状态机 bug | 🔧 排查中 | 长按切歌偶发异常（`4c32c4f`） |
| 3 | 输入捕获与 WS2812E 共用 TIM3_CH3 | ⏸️ 已规避 | `in_cap_ini` 注释禁用，红外改为 EXTI 解码 |
| 4 | 云端下行与周期上报的 busy 冲突 | ⚠️ 有分析 | main.c 注释已分析：下行命令可能被 busy 吃掉，当前以轮询顺序规避 |
| 5 | 音乐播放默认关闭 | ⏸️ 已注释 | `Audio_MusicPlay()` 默认注释，由云端/红外命令触发 |
| 6 | 字库 DMA 下载默认关闭 | ⏸️ 已注释 | `Font_Load()` 注释，省启动时间 |
| 7 | 源码编码混用 | ⚠️ 需注意 | main.c 为 GBK，api/ 部分为 UTF-8，跨工具编辑易乱码 |
...
*2026-08 更新 · 共 165 个提交 · 由 git 提交历史与源码注释整理*
